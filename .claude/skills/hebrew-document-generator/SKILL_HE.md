...
- ייבוא python-bidi: מאז python-bidi 0.5.0 הייבוא הקנוני הוא `from bidi import get_display` (הנתיב הישן `from bidi.algorithm import get_display` הוסר). הגרסה הנוכחית היא 0.6.x, ששחזרה מודול algorithm מקביל אבל השאירה את הייבוא העליון כמומלץ. גרסה 0.6.x גם הפסיקה לתמוך בפייתון מתחת ל-3.9.
...
| דרישות חשבונית מס בישראל | https://he.wikipedia.org/wiki/%D7%97%D7%A9%D7%91%D7%95%D7%A0%D7%99%D7%AA_%D7%9E%D7%A1 | שדות חובה בחשבונית מס; כדאי להצליב מול כללי רשות המסים העדכניים |
