...
<script>location.replace(${JSON.stringify(target)});</script></div></body></html>`;
      ...
      if (request.method === 'POST' && path === '/copywriter') {
        ...
        // Factual brief so the AI writes with real knowledge of the business, not blind rewrites.
        const brief =
        ...
        if (isSEO) {
          ...
          system = brief +
            ...
            'אל תשתמשי בקו מפריד ארוך (em dash, התו —) — השתמשי בפסיק, נקודה או מקף רגיל. ' +
          ...
          system = brief +
            ...
            'אל תמציאי שמות של אנשים או לקוחות, ולעולם אל תחליפי את שם המאפרת "ירדן דמרי". ' +
            'אל תשתמשי בקו מפריד ארוך (em dash, התו —). כתבי בעברית טבעית עם פסיקים, נקודות או מקף רגיל, כמו שאדם כותב. ' +
        ...
        // Safety net: strip em dashes (look AI-generated) → comma, even if the model ignored the rule.
        suggestions = suggestions.map(x => String(x).replace(/\s*—\s*/g, ', ').replace(/\s+,/g, ',').replace(/,\s*,/g, ', ').trim());
