...
## 2026-06-23 — Hero #1 SOLVED: generic blurred-fill backdrop + contained crisp foreground (desktop)
Root cause: two competing `@media(min-width:1081px)` rules — `styles.css:449` set the hero media to `object-fit:contain` but `:565` overrode it with `object-fit:cover !important` (higher specificity). So desktop ran `cover`, upscaling a 720×1280 (480p video) portrait ~2–3× across a 1440px frame → soft/pixelated + face-only crop.
Solution (Multi-Agent: PM/UX/FE/Perf/SEO, Tech-Lead green-light) — generic, works for ANY admin-chosen photo/video, zero per-asset work:
- Desktop ≥1081px: foreground media `object-fit:contain` (whole portrait, crisp, no upscale) + a new blurred backdrop layer `.hero-bg` (`object-fit:cover` + `blur(34px) brightness(.55) scale(1.18)`) fills the sides with a blurred copy of the same media → no black bars, premium Reels/Shorts-style look.
- Backdrop is a STILL image (poster/thumbnail) even for video heroes — a 2nd decoding `<video>` would double CPU/LCP for no visible gain (blur hides the low res).
- Mobile (<1081px) unchanged: backdrop `display:none`, foreground stays `cover` full-bleed (portrait media matches portrait viewport).
- Wired into `applyHeroMediaFromState()`: `#heroBg.src` = image (image hero) or poster thumb (video hero); default baked into HTML + added `poster` to the default `<video>`.
Files: `preview/index.html` (hero markup + JS), `preview/styles.css` (`.hero-bg` + desktop block).
Verified: computed styles (mobile bg=none/cover, desktop bg=block/contain) on the served page + a local render with the real 720×1280 portrait shows crisp centered subject with blurred fill (R2 media can't load inside the headless browser, so verified the technique with the asset served locally).
